...
const test = require('../settings/setting')
    //console.log(user)
    //#region Access SO Components
class __SOACCESS {
    constructor(file, musicIcon, videoIcon, folderIcon, imageIcon, textIcon, _extensions) {
        this.file = [];
        this._extensions = null;
        this.musicIcon = `data:image/svg+xml;base64,
        ...
        this.videoIcon = `data:image/svg+xml;base64,
        ...
        this.folderIcon = `data:image/svg+xml;base64,
        ...
        this.imageIcon = `data:image/svg+xml;base64,
        ...
        this.textIcon = `data:image/svg+xml;base64,
        ...
        this.__getExtensions((e) => {
            this._extensions = e;
        })
    }

    listDrives() {
        var ret = spawn.exec('wmic logicaldisk get name');
    ...
    _is_file_of_type(file, type) {
        let flag = false;
        this._extensions[type].map(i => {
    ...
    __read_SO_Files(_dir = 'c:/') {
        let all_directory = []

        fs.readdirSync(_dir).map(x => {
                let _data = { title: null, full_path: null, icon: null, play_target: null };
                let next = path.join(_dir, x);
                if (this.is_dir(next) == true) {
                    _data.icon = this.folderIcon
                } else {

                    let extension = path.extname(next).toString().toLowerCase();
                    if (this._is_file_of_type(extension, 'audio')) {
                        _data.icon = this.musicIcon;
                        _data.play_target = 'audio';
                    } else if (this._is_file_of_type(extension, 'video')) {
                        _data.icon = this.videoIcon;
                        _data.play_target = 'video';
                    } else if (this._is_file_of_type(extension, 'img')) {
                        const img = `${_dir}${x}`,
                            buffImage = fs.readFileSync(img),
                            base64data = buffImage.toString('base64');
                        _data.icon = `data:image/png;base64,${base64data}`;
                        _data.play_target = 'img';
                    } else if (extension === '.txt' || extension === '.css') {
                        _data.icon = this.textIcon;
                        _data.play_target = 'txt';
                    } else {
                        _data.icon = this.folderIcon;
                        _data.play_target = 'folder';
                    }
                }
                _data.title = x; //name of the file (folder)
                _data.full_path = `${_dir}${x}`; //Full directory
                all_directory.push(_data);
            })
            //let _file_result = fs.readdirSync(_dir);
        return all_directory;
    }


    async __getExtensions(callback) {

        var fileStream = fs.createReadStream('helpers/extensions.txt');
        var data = "",
            THAT = this;
...
const __soaccess = new __SOACCESS()
module.exports = __soaccess;
//#endregion)